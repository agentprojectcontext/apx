...
<Terminal command="apx project list -l" caption="apx project list — registered projects with id, path and agent count">
...
Project-level configuration lives in `~/.apx/projects/<apx_id>/config.json` and overrides the
global `~/.apx/config.json` for that project only. It is machine-local — never in the repo — so
a provider key set here cannot be committed. Use dotted-key syntax:
...
<Aside type="caution">
  Do not put secrets in `.apc/project.json` — that one IS committed, and the daemon refuses
  credential-shaped keys there. The project config above, and `~/.apx/config.json`, are both
  machine-local and are where a key belongs.
</Aside>
