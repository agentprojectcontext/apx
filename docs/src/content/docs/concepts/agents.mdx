...
|---|---|---|
| Has tools? | Yes — full registry | Yes — only the agent's `tools:` allowlist (not the full registry). Empty field → safe read/search/memory default. `exec_agent` runs the loop; `/agents/:slug/chat` is still text-only. |
| Execution loop | Multi-iteration tool loop | Multi-iteration when invoked as `exec_agent`; one-shot if `allowed_tools: []` |
