...
|---|---|---|
| ¿Tiene herramientas? | Sí — registro completo | Sí — solo allowlist `tools:` del agente (no el registro entero). Campo vacío → default lectura/búsqueda/memoria. `exec_agent` corre el loop; `/agents/:slug/chat` sigue siendo solo texto. |
| Loop de ejecución | Loop de herramientas multi-iteración | Multi-iteración en `exec_agent`; una sola llamada si `allowed_tools: []` |
