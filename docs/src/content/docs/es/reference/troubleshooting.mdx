...
**Problema:** `apx exec "hello"`, un chat web o un mensaje de Telegram vuelve sin una respuesta real.

**Lo que ves en realidad:** no es silencio. Todas las superficies tienen un piso de nunca-callar:
cuando un turno termina sin nada que decir, se le pide al agente que escriba el cierre a partir de
lo que el turno hizo, y una línea fija y corta ("Listo.") sale solo si tampoco pudo escribirla — lo
cual ya es una señal, porque significa que dos llamadas seguidas volvieron vacías. Un turno que
hubo que cerrar así queda anotado en el log:

```bash
apx log --tail 200 | grep "closed by the floor"
```

Un "Listo." pelado sin nada detrás, o un cierre que describe un trabajo que no pediste, es este
síntoma — las causas de abajo son lo que hay que revisar.
...
**Causa:** Una llamada parcial a `writeConfig()` (desde `apx config set --global`, una reejecución del wizard o una recarga
...
2. Si ejecutaste recientemente `apx config set --global engines.<something>`, verificá que el bloque completo todavía se vea
   bien:

   ```bash
   apx config show --global | grep -A 20 '"engines"'
   ```
...
<Aside type="caution">
  Evitá `apx config set --global engines` con un valor JSON parcial — definí siempre las claves hoja (p. ej.
