...
runtime memory, the long-lived context the agents read on every turn. It is also
where the super-agent's `remember` tool writes when it is told a fact about this
project. The super-agent's own notebook (`~/.apx/memory.md`) is global rather
than per-project, so it is listed only in **Base**.
