...
channel a conversation can happen on, grouped, and a chip row switches each one
off and back on for that device — again with Telegram off by default on the
phone. A channel that is off keeps its chip (with its count), so the way back is
where the way out was, and a list emptied by a filter says so instead of
claiming there is nothing there. On the phone, where there are no channel
headings, every row wears its channel as a tag.
