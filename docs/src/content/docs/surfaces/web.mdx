...
row. A project has two: **Project memory** (`.apc/memory.md`), curated and
committed with the repo, and **Local memory** (`~/.apx/projects/<id>/memory.md`),
which the super-agent's `remember` tool writes and git never sees — so a fact
picked up mid-conversation cannot put a pasted credential in your history.
Promoting a line from local to committed is a manual edit here. Below them sits
