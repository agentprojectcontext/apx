...
1. **By key** — structured metadata fields whose name looks secret (`token`, `api_key`,
   `authorization`, `bot_token`, …) are replaced with `[redacted]`.
2. **By value** — at boot (and again whenever the config changes) the daemon registers every
   ...
   with a `***…XXXX` marker (last 4 characters kept so you can tell which secret it was).
...
(for code/prompt changes). The `apx config set --global` and `apx model key` commands write this file and
