...
the daemon over HTTP. See [Architecture](/apx/start/architecture/) for the full picture.

<Screenshot
  surface="terminal"
  caption="apx daemon status — port, pid, uptime, plugins, projects"
...
<Screenshot
  surface="terminal"
  caption="apx log -f — live unified log stream from all modules"
  hint="Run: apx log -f" />
...
See [Routines](/apx/capabilities/routines/) for the full spec and `apx routine add` flags.
...
- ~/.apx/
  ...
  - projects/
    - \<project-id\>/
      - project.db  ← regenerable SQLite index
      - messages/
      - agents/
        - \<slug\>/
...
<Aside type="note">
  ...
  repo under `.apc/`. See [Architecture](/apx/start/architecture/) for the storage split rationale.
</Aside>
