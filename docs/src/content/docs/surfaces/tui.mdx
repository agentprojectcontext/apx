...
<Terminal command="apx code" caption="apx code — full-screen TUI with chat transcript and input bar">
...
<Aside type="note">
  When `--agent <slug>` is passed, the TUI routes chat to the project agent's `/api/agents/:slug/chat`
...
<Terminal command="apx chat reviewer" caption="apx chat reviewer — readline REPL with agent prompt">
