...
Engines are tried in order until one draws. The chain covers both ways an engine can let a
...
When every engine fails, the error names each one it tried, where that server lives and what
the server itself said, so a broken box is identifiable from the message alone:

<Terminal command={`apx image "a fox"`}>
