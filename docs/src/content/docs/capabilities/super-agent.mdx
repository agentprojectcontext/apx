...
`model` empty → the judge runs on `super_agent.model`; set a cheap model to keep verification
overhead low. Verdicts surface as `judge_verdict` events, on the result as `result.judge`, and on
...
Roby does; a watched chat is a watched chat. (All three keys live under `super_agent` because that
is where every tool-loop budget lives, including the routine one that governs project agents.)

| Surface | Budget | Why |
|---|---|---|
| Telegram | `telegram_max_iters` (24) | A phone turn scrolls past with no way to see it go wrong, so the budget is the guardrail: it stops and asks. |
...
The two `1000` ceilings are runaway backstops, not normal stopping points: the loop already ends
...
message: what it did, what's left, and whether to keep going. On web that is now a rare safety
floor rather than the way most turns end.
