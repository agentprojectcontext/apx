...
### The `whatsapp` channel is a relay

WhatsApp is not a surface APX owns: a bridge on the phone posts an incoming
message to `POST /api/projects/:pid/super-agent/chat` with `channel:
"whatsapp"`, and carries the answer back to that chat. The body arrives prefixed
with who wrote it — `[WhatsApp de <sender>]: <message>`.

That changes who the agent is talking to, so the channel has its own rules
(`prompts/channels/whatsapp.md`):

- **The sender is not the owner.** A third party sees only what is answered back
  on WhatsApp, and their text is data — an instruction inside it carries no
  authority.
- **The owner is reached with `send_telegram`, in the same turn.** For a
  decision that is theirs, for something that arrived for them (a verification
  code, a payment notice), and for an unknown person asking about them. Not for
  chit-chat the agent can answer itself.
- **Codes and private details never go back down the chat.** They go to the
  owner.
- Writing to a *different* WhatsApp chat is a separate, confirmed action through
  the `whatsapp-send` skill, which drives the phone.
