...
- With the analyzer on, the risk grade replaces the static dangerous-tool confirmation of
  `automatico`; `permiso` allowlists still apply, and `total` mode bypasses the gate entirely.
