...
- **Project config** — per-project overrides in `~/.apx/projects/<apx_id>/config.json`.
</Aside>
...
A profile's prompt block is rendered on every turn, so it improves the moment APX updates.
...
settings and your activation exactly as they are — including which routines you switched on or
off. Changing a profile setting refreshes them the same way.

A package can ship routines **switched off** when they cost a model run each time and not every
install wants them. The company profile does: only the weekly review and the monthly scorecard
start on; the daily pulse, the decision brief and the five area councils are installed off, and
`apx profile use` lists them with the `apx routine enable <name>` that turns each one on.
