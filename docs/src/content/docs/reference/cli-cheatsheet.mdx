...
<Terminal command="apx --help" caption="apx --help — trimmed to the first and last of its 24 sections; the real output lists every command in full">
...
| `apx agent set <slug>` | Edit the system prompt (`--prompt -`) or any other field |
| `apx agent list` | List project agents |
| `apx agent get <slug>` | Print one agent definition |
| `apx agent remove <slug>` | Delete a project agent (file + runtime memory) |
...
| `apx config show --global` | Print `~/.apx/config.json`, secrets redacted |
| `apx config set <key.path> <value>` | Set a JSON-aware value in the project's config |
| `apx config set --global <key.path> <value>` | Set a value in `~/.apx/config.json` — credentials, voice, daemon |
| `apx config unset [--global] <key.path>` | Remove a key from the project or global config |
...
| `apx memory notebook` | Show the super-agent's own notebook (`~/.apx/memory.md`) and its size |
| `apx memory consolidate [--apply]` | Propose durable facts from stdin candidates (writes only with `--apply`) |
| `apx memory prune [--apply]` | Collapse replicated notebook entries, keeping the newest (dry-run by default) |
| `apx memory revert [--since D]` | Remove entries written by consolidation |
...
| Command | What it does |
|---|---|
| `apx obsidian set <vault-path>` | Connect a vault, validating the path (`--global --mcp --memory`) |
| `apx obsidian status` | Show the configured vault, note count and toggles for the scope |
| `apx obsidian sync` | Mirror APX memory into the vault, without duplicates |
| `apx obsidian remove` | Disconnect the vault (and any auto-registered MCP) |

Scope works like the rest of APX: the command targets the current project unless
`--global` (or `--scope global`) puts the vault in the space every project shares.
`--mcp` registers an `obsidian` MCP server pointing at it; `--memory` turns on memory
sync, which then runs on `apx obsidian sync`.
...
| `apx usage` | Model calls for a UTC day by model, channel and agent (`--date`, `--since H`, `--json`) |
| `apx usage breaker` / `resume` | The spend breaker's last hour and pause; lift a pause now |
...
| `apx acp` | Serve the super-agent over the Agent Client Protocol (stdio, for IDEs) |
...
| Command | What it does |
|---|---|
| `apx send <from> <to> "msg"` | Log an A2A message (`--deliver` runs a project agent, runtime, or the configured super-agent identity) |
...
What you promised a named person, by when. Separate from tasks: a task is work,
a commitment is someone waiting.

| Command | What it does |
|---|---|
| `apx commitment add "<what>" --to "<person>"` | Record a promise (`--due ISO --project X --channel C`) |
| `apx commitment list` | List promises (`--all --to X --state open\|kept\|missed\|dropped\|all --overdue`) |
| `apx commitment show <id>` | Print one promise, with every date it has had |
| `apx commitment kept <id>` | Delivered (`--note "..."`) |
| `apx commitment missed <id>` | The date passed and it did not happen (`--note "..."`) |
| `apx commitment renegotiate <id> --due <ISO>` | A new date, agreed with them — reopens it and keeps the old date |
| `apx commitment drop <id>` | Filed by mistake — off the lists, and NOT counted as broken |
...
| `apx artifact preview <name>` | Serve HTML/React on a local URL with live reload (`--open --share`) |
| `apx artifact share <name>` | Preview plus a public tunnel URL |
| `apx artifact previews` | List running previews (stop one with `apx artifact stop <id>`) |
...
| Command | What it does |
|---|---|
| `apx image "<prompt>"` | Generate an image (`--out <file> --size 768x512 --steps N --cfg N --seed N`) |
| `apx image "<prompt>" --provider <id>` | Force an engine: `a1111` \| `sdcpp` \| `openai` \| `mock` \| `custom:<slug>` |
| `apx image providers` | Which image engines are configured, reachable and in what order |
| `apx image capabilities` | Models, samplers and schedulers a server offers (`--provider <id>`) |
...
| `apx voice listen` | Mic → `/api/voice/turn` → reply (`--seconds N --no-play`) |
| `apx voice providers` | List configured TTS / STT providers |
| `apx transcribe <file…\|dir>` | STT of audio/video through the warm daemon whisper; bulk-capable (`--lang es --provider <id> --json`) |
...
<CardGrid>
  <LinkCard title="CLI surface" href="/apx/docs/surfaces/cli/" description="Shell completion, config file, and interactive usage patterns." />
  <LinkCard title="Engines & runtimes" href="/apx/docs/engine/runtimes/" description="Full guide to runtimes (claude-code, codex, …) and apx run." />
  <LinkCard title="Configuration" href="/apx/docs/concepts/configuration/" description="~/.apx/config.json reference — engines, providers, model fallback." />
  <LinkCard title="Troubleshooting" href="/apx/docs/reference/troubleshooting/" description="Daemon issues, failed providers, Telegram conflicts, and more." />
</CardGrid>
