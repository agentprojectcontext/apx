...
**Problem:** `apx exec "hello"`, a web chat or a Telegram message comes back with no real answer.

**What you actually see:** not silence. Every surface has a never-silent floor: when a turn ends
with nothing to say, the agent is asked to write the closing line from what the turn did, and a
short canned line ("Done.") goes out only when it cannot answer either — which is itself a signal,
because it means two calls in a row came back empty. A turn that had to be closed that way says so
in the log:
...
A bare "Done." with nothing behind it, or a closing that describes work you did not ask for, is
this symptom — the causes below are what to check.
...
**Cause:** A partial `writeConfig()` call (from `apx config set --global`, a wizard re-run, or a daemon
...
2. If you recently ran `apx config set --global engines.<something>`, verify the full block still looks
   right:
...
<Aside type="caution">
  Avoid `apx config set --global engines` with a partial JSON value — always set leaf keys (e.g.
