...
Background work — routines and agents talking to each other — has an hourly ceiling on model calls,
counted over the last hour, globally and per project. Past it, that work pauses (30 minutes by
default) and you get one Telegram line saying so; your own chats are never paused. The limits live in
