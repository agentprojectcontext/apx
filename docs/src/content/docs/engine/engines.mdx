...
turn, and turns nobody is watching (a2a, routines) stop rather than fall through — see
[the super-agent page](/apx/docs/capabilities/super-agent/). For the ChatGPT plan, reasoning effort
goes on the model id — `chatgpt-codex:gpt-5.6-luna@medium` (`minimal|low|medium|high|xhigh`) — or
as a default in `engines.chatgpt-codex.reasoning_effort`.
