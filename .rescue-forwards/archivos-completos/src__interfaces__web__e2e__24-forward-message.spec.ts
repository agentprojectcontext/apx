...
async function stubChat(page: Page, pid: number, messages = MESSAGES) {
  ...
  await page.route(
    ...
    (route) =>
      route.fulfill({
        json: [{ id: CONV, title: "Acme", messages: messages.length, updated_at: nowIso(), created_at: nowIso(60) }],
      }),
  );
  await page.route(
    ...
    (route) =>
      route.fulfill({
        json: [{ id: "acme-01", title: "Envíos", messages: 4, updated_at: nowIso(120), created_at: nowIso(600) }],
      }),
  );
  await page.route(
