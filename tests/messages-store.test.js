...
test("parseDayJsonl reads JSONL", async () => {
  const { parseDayJsonl } = await import("#core/stores/messages.js");
  const text = [
...
test("getRecentTelegramTurns — pulls per chat_id in chronological order", async () => {
  ...
  const db = freshDb();
  try {
    const append = (await import("#core/stores/messages.js")).appendMessage;
...
test("getRecentTelegramTurns — respects max_age_hours and limit", async () => {
  const { getRecentTelegramTurns, appendMessage } = await import("#core/stores/messages.js");
  const root = makeTempProject({ agents: [{ slug: "sofia" }] });
  const db = freshDb();
  try {
...
test("getRecentTelegramTurns — sanitizes assistant turns with factual data", async () => {
  ...
  try {
    ...
    // The assistant turn was redacted (model ids + bullet list), and what
    // replaces it is a note on the SYSTEM side. Not a wording preference: while
    // the annotation rode in the assistant's own voice, the window filled with
    // copies of one sentence and the model said it back to the user.
