...
1. **Tests ship with behavior.** Every new route/command/plugin/config key and every bug fix lands with a test in `tests/`. Drive HTTP through `buildApi()` + `app.listen(0, "127.0.0.1")` — **with the host**, because a bare `app.listen(0)` binds the wildcard, leaves an unauthenticated API on every interface for the run, and lets another test file bind the same port on top of it and answer your requests ([`rules/testing.md`](rules/testing.md)); build trees with `makeTempProject()`. Anything writing under `~/.apx` must set **`process.env.APX_HOME`** to its own temp dir **before** importing the module — not `HOME` alone. The `test:ci`/`test` runners pin one shared `APX_HOME` sandbox for the whole run, and `computeHome()` checks `APX_HOME` **before** falling back to `os.homedir()`, so a test that only moves `HOME` silently shares that one sandbox and races every other such test in parallel (this cost a run of memory/telegram/inbox flakes). Set `process.env.APX_HOME = path.join(tmpHome, ".apx")` (keep `HOME`/`USERPROFILE` too for `~/.claude`-style lookups). Tests run offline: no network, no keys, no live daemon.
...
6. **Skills stay in sync.** When you change CLI commands, routes, config keys, or behavior documented in a skill, update the matching `SKILL.md` in the same change. **Look in `src/core/runtime-skills/<slug>/` first** — that's where the ~19 skills the super-agent actually loads live. The 3 under `skills/` are only the bundled ones shipped to npm, and `skills/apc-context/` is synced from upstream (don't hand-edit it). Verify flags with `apx <command> --help` — don't invent subcommands.
   - **A changed default or semantic is a change too — and it rarely lives in one skill.** When you flip a shared behavior (a tool-permission default, the meaning of a routine field, an engine/runtime list, a scope rule), `grep` every `src/core/runtime-skills/*/SKILL.md` for the old claim before you finish: one such change — e.g. the capability-by-default tools reversal, which inverted `empty tools = no tools` — silently falsified three skills at once. `tests/runtime-skills.test.js` guards each skill's header, `name`==dir and English-only prose, but **not** whether the prose is still true; that check is on you and on review.
