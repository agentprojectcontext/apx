...
export const en = {
  ...
  settings_ui: {
    ...
    kw_triggers_title:          "Keyword triggers (option B)",
    kw_triggers_desc:           "Alternative activation, OpenHands-style. Skills that declare `triggers:` (a keyword list) in their frontmatter are auto-injected when a keyword appears in the user's message (case-insensitive substring). On a match, the inspector/RAG step is skipped for that turn. Off by default — the semantic inspector remains the primary mechanism.",
    enable_kw_triggers:         "Enable keyword triggers",
    enable_kw_triggers_hint:    "Off = skills activate via /slug, the inspector, or the passive suggestion. On = a keyword match also auto-injects the skill body.",
    kw_max_matches:             "Max. matches",
    kw_max_matches_hint:        "How many skills can be injected by keyword per turn.",
    kw_body_char_cap:           "Body char cap",
    kw_body_char_cap_hint:      "Trims long skill bodies so they don't bloat the context.",
    kw_skills_title:            "Skills with triggers",
    kw_no_skills:               "No skill declares `triggers:` in its frontmatter yet. Add e.g. `triggers: [deploy, release]` to a SKILL.md.",
