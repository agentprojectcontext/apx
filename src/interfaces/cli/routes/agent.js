...
import { cmdAgentAdd, cmdAgentGet, cmdAgentImport, cmdAgentList, cmdAgentPacks, cmdAgentRemove, cmdAgentSet, cmdAgentVaultAdd, cmdAgentVaultList, cmdAgentVaultRestore, cmdAgentVaultRm } from "../commands/agent.js";
...
export default async function route(rest, { parseArgs, die }) {
  ...
  else if (sub === "packs" || sub === "pack") await cmdAgentPacks(a);
  else if (sub === "vault") {
    const vsub = a._[0];
  ...
  else die(`unknown agent subcommand: ${sub || "(none)"} — try: add, set, list, get, remove, import, packs, vault`);
}
