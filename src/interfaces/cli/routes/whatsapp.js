...
import { cmdWhatsAppChats, cmdWhatsAppFollowUp, cmdWhatsAppRepair, cmdWhatsAppStatus } from "../commands/whatsapp.js";
...
export default async function route(rest, { parseArgs, die }) {
  ...
  else if (sub === "follow-up") await cmdWhatsAppFollowUp(a, { die });
  else die(`unknown whatsapp subcommand: ${sub} — try: status, chats, repair, follow-up`);
}
