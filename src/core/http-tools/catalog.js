...
export const TOOL_DEFINITIONS = Object.freeze([
  ...
  // ── browser (Puppeteer-backed — heavier, launches Chromium lazily) ────────
  {
    ...
    category: "browser",
    description: "Navigate the headless browser to a URL. Launches Chromium lazily on first call. Auto-retries and falls back to a more permissive wait strategy on redirect-heavy sites. Pass snapshot:true to get the page's accessibility tree back in the same call — that is the normal way to drive a page: navigate → read refs → act on refs.",
    ...
    parameters: {
      ...
      properties: {
        ...
        snapshot: { type: "boolean", description: "Also return the accessibility snapshot (with refs) after the page settles. Saves a round trip." },
    ...
    examples: [{ url: "https://example.com", snapshot: true }],
  },
  {
    name: "browser_snapshot",
    category: "browser",
    description: "Read the current page as an accessibility tree: role, accessible name, state, one line per element, with every interactive control stamped [ref_N]. THIS is how you see a page — call it after navigating and after any action that changes the DOM, then pass a ref to browser_click / browser_type / browser_select / browser_hover instead of guessing a CSS selector from HTML. Refs are invalidated by navigation or re-render; take a fresh snapshot when an action reports a stale ref.",
    endpoint: { method: "POST", path: "/api/tools/browser/snapshot" },
    parameters: {
      type: "object",
      properties: {
        selector:         { type: "string",  description: "Limit the snapshot to a subtree (e.g. \"main\", \"#results\"). Use it on big pages." },
        interactive_only: { type: "boolean", description: "Only controls — drop headings and page copy. Smallest output.", default: false },
        max_nodes:        { type: "number",  description: "Node cap before truncation (default 400, max 1500).", default: 400 },
      },
    },
    examples: [{}, { selector: "form" }, { interactive_only: true }],
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        ref:         { type: "string",  description: "Snapshot ref (e.g. \"ref_12\") of the element to capture. Wins over selector." },
  ...
  },
  {
    name: "browser_click",
    category: "browser",
    description: "Click an element on the current browser page. Prefer `ref` from browser_snapshot — a CSS selector guessed from HTML is what makes browsing brittle.",
    endpoint: { method: "POST", path: "/api/tools/browser/click" },
    parameters: {
      type: "object",
      properties: {
        ref:      { type: "string",  description: "Snapshot ref (e.g. \"ref_12\"). Preferred." },
        selector: { type: "string",  description: "CSS selector, when no snapshot ref applies." },
        snapshot: { type: "boolean", description: "Return the accessibility snapshot after the click. Saves a round trip." },
      },
    },
    examples: [{ ref: "ref_12" }, { ref: "ref_12", snapshot: true }, { selector: "button#submit" }],
  },
  {
    name: "browser_type",
    category: "browser",
    description: "Type text into a field. Prefer `ref` from browser_snapshot. Uses focus + Ctrl+A + Backspace to clear, then types with realistic delay.",
    endpoint: { method: "POST", path: "/api/tools/browser/type" },
    parameters: {
      type: "object",
      properties: {
        ref:      { type: "string",  description: "Snapshot ref (e.g. \"ref_3\"). Preferred." },
        selector: { type: "string",  description: "CSS selector, when no snapshot ref applies." },
        text:     { type: "string" },
        clear:    { type: "boolean", default: true },
        snapshot: { type: "boolean", description: "Return the accessibility snapshot after typing." },
      },
      required: ["text"],
    },
    examples: [{ ref: "ref_3", text: "hello world" }, { selector: "input#search", text: "hello world" }],
  },
  {
    name: "browser_select",
    category: "browser",
    description: "Choose an option in a <select> element by its value. Prefer `ref` from browser_snapshot.",
    endpoint: { method: "POST", path: "/api/tools/browser/select" },
    parameters: {
      type: "object",
      properties: {
        ref:      { type: "string",  description: "Snapshot ref of the <select>. Preferred." },
        selector: { type: "string",  description: "CSS selector, when no snapshot ref applies." },
        value:    { type: "string" },
        snapshot: { type: "boolean", description: "Return the accessibility snapshot after selecting." },
      },
      required: ["value"],
    },
    examples: [{ ref: "ref_7", value: "AR" }, { selector: "select#country", value: "AR" }],
  },
  {
    name: "browser_hover",
    category: "browser",
    description: "Hover the cursor over an element (triggers tooltips, dropdowns, hover states). Prefer `ref` from browser_snapshot.",
    endpoint: { method: "POST", path: "/api/tools/browser/hover" },
    parameters: {
      type: "object",
      properties: {
        ref:      { type: "string", description: "Snapshot ref. Preferred." },
        selector: { type: "string", description: "CSS selector, when no snapshot ref applies." },
      },
    },
    examples: [{ ref: "ref_5" }, { selector: "nav .menu-item" }],
  },
  {
  ...
  },
  {
    ...
    category: "browser",
    description: "Return raw innerHTML of the page or a single element (truncated at 1MB). Expensive — prefer browser_snapshot to find elements and browser_get_text to read copy; reach for raw HTML only when you need the markup itself.",
