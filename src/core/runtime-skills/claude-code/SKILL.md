...
## Not here: agent-to-agent (a2a) hand-off

Reaching an APX agent and being answered back (the a2a relay: `apx send … --deliver`,
`engine=… session=…` identity, `apx session resume … --msg`) is a **harness-side**
concern. It lives in the `apx` skill that every coding CLI loads, and on the agent
side in **apx-runtime**. This driver covers only how APX operates the Claude
engine — keep a2a out of it.
