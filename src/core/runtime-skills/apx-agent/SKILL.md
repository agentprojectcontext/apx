...
| `remove_agent` | Deletes the agent + its runtime data | Irreversible — confirm first unless clearly asked. Master/orchestrator agents and the super-agent only, same as `rename_agent`. |
...
**Who may rename or delete.** Both `rename_agent` and `remove_agent` reshape another agent's existence, so they follow the ROLE: an agent with `type: orchestrator` (or `is_master`), and the super-agent. A specialist does not get them, and declaring them on its card does not change that — the switch is the role. An agent that reaches for one anyway gets it recorded on the `log` channel, so the attempt is visible rather than silent.
...
- **A pinned model that fails** (429, 5xx) falls down the router's fallback chain — router #1 first — and does not die. `apx agent set <slug> --no-model-fallback` (frontmatter `model_fallback: false`, panel switch "If it fails, continue down the router chain") makes it strict: it fails instead. A spent plan ("usage limit") on a pinned model with fallback on also walks on to the router, even on a turn nobody is watching; only the ROUTER's own account running dry stops an unwatched turn (a2a, routine) and is reported.
- **ChatGPT (`chatgpt-codex:`) effort** is a setting of the model, not another model (the panel has an Effort control and shows `gpt-5.6-luna · high`); it is stored on the id: `chatgpt-codex:gpt-5.6-luna@medium` (`minimal|low|medium|high|xhigh`). Without a suffix, `engines.chatgpt-codex.reasoning_effort` applies, else the backend default.
